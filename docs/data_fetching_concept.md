...
6. **Indexdatenbanken**: Für die spätere Auswahl und Synchronisation existieren zwei gleich strukturierte SQLite-DBs. Der **Online-Index** wird ohne Dokumentdownloads erzeugt: `scripts/build_online_index_db.py` lädt Monats- und Detailseiten, schreibt nach `data/processed/online_session_index.sqlite` und speichert nur Metadaten (Session, TOPs, Dokument-URLs). Mit `--refresh-existing` werden vorhandene Sitzungen aktualisiert, mit `--only-refresh` werden ausschließlich bestehende Sitzungen erneuert. Der **lokale Index** wird aus bereits geladenen Daten aufgebaut: `scripts/build_local_index.py` liest `data/raw/` und schreibt nach `data/processed/local_index.sqlite`.

...
- `/daten/build/` startet vorhandene SQLite-Build-Skripte für Ratsinfo- und Landkreis-Indizes. Der lokale Ratsinfo-Build erneuert eine veraltete `agenda_summary.json` automatisch aus einer neueren `session_detail.html`; der Online-Build verändert `data/raw/` nicht.

...
- `/daten/build/` startet vorhandene SQLite-Build-Skripte für Ratsinfo- und Landkreis-Indizes. Der lokale Ratsinfo-Build erneuert veraltete TOP- und Dokumentmetadaten automatisch aus einer neueren `session_detail.html`; Dokumentanzahl und lokale Verfügbarkeit bleiben dabei getrennt. Der Online-Build verändert `data/raw/` nicht.

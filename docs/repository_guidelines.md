...
- `scripts/build_local_index.py` erzeugt einen lokalen Index aus bereits heruntergeladenen Rohdaten (`data/raw/`) und schreibt standardmäßig nach `data/db/local_index.sqlite`. Eine neuere `session_detail.html` wird vor dem Import geparst und mit `agenda_summary.json` synchronisiert.
